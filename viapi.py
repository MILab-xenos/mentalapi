...
class cconverter:
    """Colorspace conversion chain (只保留解码所需部分)."""
...
def surface_to_tensor(surface: nvc.Surface) -> torch.Tensor:
    ...
    img_tensor = pnvc.DptrToTensor(
        surf_plane.GpuMem(), surf_plane.Width(), surf_plane.Height(),
...
def process_video_pipeline(
    ...
    # 若无法直接读取帧数，空转获取总数
    if total_frames <= 0:
        temp_dec = nvc.PyNvDecoder(encFilePath, GPU_ID)
        true_frames = 0
        while True:
            try:
                if temp_dec.DecodeSingleSurface().Empty(): break
                true_frames += 1
    ...
    try:
        while True:
            try:
                ...
                if src_surface.Empty(): break
            except RuntimeError as e:
                print(f"Decode error at frame {frame_idx}: {e}")
                continue
