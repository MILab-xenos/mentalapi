...
def generate_prior_boxes_new(min_sizes, steps, clip, image_size, device="cpu"):
    """
    Optimized prior box generation using vectorized tensor operations,
    fixed to match the output order of the original function.
    """
    anchors_all_levels = []
    image_h, image_w = image_size # image_size is (height, width)

    for k, step in enumerate(steps):
        # min_sizes_k for the current feature level
        current_min_sizes = torch.tensor(min_sizes[k], dtype=torch.float32, device=device)
        num_anchor_types_per_loc = len(current_min_sizes)

        feat_h = int(torch.ceil(torch.tensor(image_h / step)))
        feat_w = int(torch.ceil(torch.tensor(image_w / step)))

        # Create meshgrid for center points for this feature map
        # grid_y: (feat_h, feat_w), grid_x: (feat_h, feat_w)
        grid_y, grid_x = torch.meshgrid(
            torch.arange(feat_h, dtype=torch.float32, device=device),
            torch.arange(feat_w, dtype=torch.float32, device=device),
            indexing="ij"  # ensures grid_y[i,j]=i, grid_x[i,j]=j
        )

        # Calculate base center coordinates (normalized) for each cell in the feature map
        # Shape: (feat_h, feat_w)
        cx_map = (grid_x + 0.5) * step / image_w
        cy_map = (grid_y + 0.5) * step / image_h

        # Expand cx_map and cy_map to accommodate different anchor types at each location
        # Shape: (feat_h, feat_w, 1) -> (feat_h, feat_w, num_anchor_types_per_loc)
        cx_map_expanded = cx_map.unsqueeze(-1).repeat(1, 1, num_anchor_types_per_loc)
        cy_map_expanded = cy_map.unsqueeze(-1).repeat(1, 1, num_anchor_types_per_loc)

        # Calculate s_kx and s_ky for each min_size
        # Shape: (num_anchor_types_per_loc)
        s_kx_vals = current_min_sizes / image_w
        s_ky_vals = current_min_sizes / image_h

        # Expand s_kx_vals and s_ky_vals to match the map dimensions
        # Reshape to (1, 1, num_anchor_types_per_loc) then expand
        # Shape: (feat_h, feat_w, num_anchor_types_per_loc)
        s_kx_map = s_kx_vals.reshape(1, 1, num_anchor_types_per_loc).expand_as(cx_map_expanded)
        s_ky_map = s_ky_vals.reshape(1, 1, num_anchor_types_per_loc).expand_as(cy_map_expanded)
        
        # Stack to form anchors for this level
        # anchors_k will have shape (feat_h, feat_w, num_anchor_types_per_loc, 4)
        anchors_k = torch.stack([cx_map_expanded, cy_map_expanded, s_kx_map, s_ky_map], dim=-1)

        # Reshape to (feat_h * feat_w * num_anchor_types_per_loc, 4)
        # The order of elements after reshape will be:
        # (loc_0_0_type_0, loc_0_0_type_1, ..., loc_0_1_type_0, loc_0_1_type_1, ...)
        # This matches the itertools.product order of the original function.
        anchors_k_flat = anchors_k.reshape(-1, 4)
        anchors_all_levels.append(anchors_k_flat)

    output = torch.cat(anchors_all_levels, dim=0)
    # output = output.to(device) # All intermediate tensors are already on device
...
def postprocess_retinaface(
    ...
    priors = generate_prior_boxes_new(
        face_config["min_sizes"],
        face_config["steps"],
        face_config["clip"],
        image_size=(im_height, im_width),
