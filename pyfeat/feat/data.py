...
def read_video(filename, start_pts=0, end_pts=None, pts_unit='pts', output_format='THWC', **kwargs):
    """
    自定义的 read_video 替代函数，使用 OpenCV 读取，
    完美模拟旧版 torchvision.io.read_video 的输出格式。
    """
    ...
    while cap.isOpened():
        ...
        if not ret:
            break
        # OpenCV 默认是 BGR 格式，需要转为 RGB 以匹配 torchvision 行为
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        frames.append(torch.from_numpy(frame))
    cap.release()
    
    if len(frames) > 0:
        video = torch.stack(frames)  # 形状为 [T, H, W, C]，类型为 uint8
    else:
        video = torch.empty((0, 1, 1, 3), dtype=torch.uint8)
        
    # 如果 pyfeat 要求 TCHW (就像你报错信息里说的那样)
    if output_format == "TCHW":
        video = video.permute(0, 3, 1, 2)
        
    # pyfeat 处理面部特征不依赖音频，直接返回空张量
    audio = torch.empty(0) 
    info = {"video_fps": fps}
