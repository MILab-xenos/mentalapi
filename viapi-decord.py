...
import numpy as np
import decord  # NVDEC GPU 硬解 (源码编译, USE_CUDA=ON)
decord.bridge.set_bridge('torch')  # get_batch/索引直接返回 torch 张量
...
class CpuVideoDecoder:
    """基于 decord NVDEC 硬解的视频解码器 (ctx=gpu, USE_CUDA 源码编译)。

    对外接口与原 nvDec 对齐：Width()/Height()/Numframes() 以及逐帧迭代。
    decord 直接把解码后的帧输出为 GPU 上的 torch 张量 (H, W, 3) uint8，
    无 GPU->CPU 拷贝；此处仅做 permute + 归一化，产出与 CPU 后端一致的
    (3, H, W)、[0,1] float GPU 张量，下游逻辑无需改动。

    命名沿用 CpuVideoDecoder 以便三套后端接口统一 (combinedapi 按名加载)。
    """
    def __init__(self, enc_file_path: str, gpu_id: int):
        ...
        self._ctx = decord.gpu(gpu_id) if torch.cuda.is_available() else decord.cpu(0)
        vr = decord.VideoReader(enc_file_path, ctx=self._ctx)
        self._num_frames = len(vr)
        frame0 = vr[0]  # (H, W, 3)
        self._h, self._w = int(frame0.shape[0]), int(frame0.shape[1])
        del vr
    ...
    def frames(self):
        """逐帧生成 GPU 上的 (3, H, W)、[0,1] float 张量 (NVDEC 硬解)。"""
        vr = decord.VideoReader(self.path, ctx=self._ctx)
        for i in range(len(vr)):
            frame = vr[i]  # torch (H, W, 3) uint8, 已在 GPU 上
            tensor = frame.to(self.torch_device)
...
if __name__ == "__main__":
    # 使用 Uvicorn 运行服务
    uvicorn.run("viapi-decord:app", host="0.0.0.0", port=10035, reload=False)