...
import numpy as np
import av  # NVDEC GPU 硬解 (PyAV hwaccel=cuda)
...
class CpuVideoDecoder:
    """基于 PyAV NVDEC 硬解的视频解码器 (hwaccel=cuda)。

    对外接口与原 nvDec 对齐：Width()/Height()/Numframes() 以及逐帧迭代。
    使用 GPU NVDEC 单元解码，失败时自动回退到 CPU 软解，保证鲁棒性。
    每帧解码为 RGB 后转成 GPU 上的 (3, H, W)、[0,1] float 张量，
    与 CPU 后端输出格式保持一致，下游逻辑无需改动。

    注意：Tesla M40 的第一代 Maxwell NVDEC 较弱，且 to_ndarray 会触发
    GPU->CPU 拷贝，实测未必快于 CPU 软解；保留此后端以便在具备强 NVDEC
    的 GPU (如 T4/A10) 上启用。
    """
    def __init__(self, enc_file_path: str, gpu_id: int):
        ...
        # 允许软解回退：编解码器不被 NVDEC 支持时不至于报错
        self._hwaccel = HWAccel(device_type="cuda", allow_software_fallback=True)
    ...
    def frames(self):
        ...
        container = av.open(self.path, hwaccel=self._hwaccel)
        try:
...
if __name__ == "__main__":
    # 使用 Uvicorn 运行服务
    uvicorn.run("viapi-pyavgpu:app", host="0.0.0.0", port=10034, reload=False)